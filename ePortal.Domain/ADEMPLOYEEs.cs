//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Manual changes to this file may cause unexpected behavior in your application.
//     Manual changes to this file will be overwritten if the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

namespace ePortal.DomainClasses
{
    using System;
    using System.Collections.Generic;
    using System.ComponentModel.DataAnnotations;
    using System.ComponentModel.DataAnnotations.Schema;

    [Table("ADEMPLOYEE")]
    public partial class ADEMPLOYEEs
    {
        [Key]
        public long ADEMPCODE { get; set; }
        public string SALUTATION { get; set; }
        public string FIRSTNAME { get; set; }
        public string LASTNAME { get; set; }
        public string CALLINGNAME { get; set; }
        public string EMAILID { get; set; }
        public System.DateTime? DOB { get; set; }
        public string GENDER { get; set; }
        public string TPHONE { get; set; }
        public string TMOBILE { get; set; }
        public string BLOODGROUP { get; set; }
        public System.DateTime REGDATE { get; set; }
        public System.DateTime EXPIRYDATE { get; set; }
        public short ACTIVE { get; set; }
        public System.DateTime DATEADDED { get; set; }
        public System.DateTime? DATELSTMODE { get; set; }
        public long ADDEDBY { get; set; }
        public long? MODIFIEDBY { get; set; }
        public string JOBTITLE { get; set; }
        public string EXTENSIONNO { get; set; }
        public string DIRECTLANDLINENO { get; set; }
        public long? LOCATION { get; set; }
        public long SYEMPLOYEETYPEID { get; set; }
        public string OPERATION { get; set; }
        public string JACKETSIZE { get; set; }
        public string TROUSERSIZE { get; set; }
        public string SHOESIZE { get; set; }
        public long? SYSITEID { get; set; }
        public string ESSTQUAL { get; set; }
        public System.DateTime? DOC { get; set; }
        public decimal? PASTEXP { get; set; }
        public string EMERGENCYCONTNO { get; set; }
        public string LOCKERNO { get; set; }
        public string SHIRTSIZE { get; set; }
        public string TSHIRTSIZE { get; set; }
        public string WARMERLWSIZE { get; set; }
        public string WARMERUPSIZE { get; set; }
        public System.DateTime? CONFIRMATIONDATE { get; set; }
        public string EXTENSIONDATE { get; set; }
        public string PANCARDNO { get; set; }
        public string CONFIRMATIONTXT { get; set; }
        public string WINTERJACKETSIZE { get; set; }
        public string MARITALSTATUS { get; set; }
        public System.DateTime? MARITALDATE { get; set; }
        public string EMPLTYPE { get; set; }
        public string PERSONALEMAILID { get; set; }
        public string JACKETHALFFULL { get; set; }
        public string MOBILE_COMPANY { get; set; }
        public string MOBILE_MODEL { get; set; }
        public string MOBILE_HEIGHT { get; set; }
        public string MOBILE_WIDTH { get; set; }
        public string MOBILE_THICKNESS { get; set; }
        public string MOBILE_COMPANY2 { get; set; }
        public string MOBILE_MODEL2 { get; set; }
        public string MOBILE_HEIGHT2 { get; set; }
        public string MOBILE_WIDTH2 { get; set; }
        public string MOBILE_THICKNESS2 { get; set; }
    }
}
