...
function upload_Click() {
    ...
    if (document.getElementById("filefinaldoc").value === "") {
        alert("Final Document is Mandatory");
        document.getElementById("filefinaldoc").focus();
        return false;
    }
...
function SubmitFinalDocumentSave() {
    var formData = new FormData();

    formData.append('hdiomid', $('#hdiomid').val() || ''); 
    formData.append('txt_Remarks', $('#txt_Remarks').val() || '');
    formData.append('txtvendorname', $('#lblvendorname').text() || '');
    formData.append('filefinaldoc', $("#filefinaldoc")[0].files[0]);

    $.ajax({
        url: '/IOMContract/SubmitFinalDocumentSave',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
