...
$(document).ready(function () {
   ...
   //// changeterm();
    ...
    if ($("#txtcontactecode").val() && $("#txtcontactecode").val().trim() !== "") {
        txtemployeecode_TextChanged($("#txtcontactecode").val());
    }
    ...
    $("#cmdSubmit").on("click", function () {
        if (upload_Click()) {
            UpdateIOMRequestForm();
        }
    });

    $("#linkbtnagreement").on("click", function (e) {
        e.preventDefault();
        var url = $(this).data('url');
        window.open(url, "_blank", 'width=800,height=550,top=90,left=200', 'resizable=yes,scrollbars=yes');
    });

    $("#linkagreedel").on("click", function (e) {
        e.preventDefault();

        $('#fileUploadAttachmentAA').prop('disabled', false);
        $('#linkbtnagreement').hide();
        $('#linkagreedel').hide();
    });

    $("#linkappnote").on("click", function (e) {
        e.preventDefault();
        var url = $(this).data('url');
        window.open(url, "_blank", 'width=800,height=550,top=90,left=200', 'resizable=yes,scrollbars=yes');
    });

    $("#linkappnotedel").on("click", function (e) {
        e.preventDefault();

        $('#fileUploadAttachmentRA').prop('disabled', false);
        $('#linkappnote').hide();
        $('#linkappnotedel').hide();
    });
...
function UpdateIOMRequestForm() {
    var formData = new FormData();

    formData.append("hdIOM", $("#hdIOM").val());
    formData.append("FileUploadAttachmentAAEnabled", $("#fileUploadAttachmentAA").prop("disabled") ? false: true);
    formData.append("FileUploadAttachmentRAEnabled", $("#fileUploadAttachmentRA").prop("disabled") ? false : true);

    formData.append("hdnagreeFile", $("#hdnagreeFile").val());
    formData.append("hdnrefFile", $("#hdnrefFile").val());
    ...
    $.ajax({
        url: '/IOMContract/IOMRequestFormUpdate',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
