...
function loadVendorRequests() {
    ...
    $.ajax({
        url: '/VendorMaster/GetApprovalRequestFin',
        type: 'POST',
        data: filter,
    ...
    if (!items || items.length === 0) {
        ...
        $.each(items, function (i, item) {
            ...
            <tr>
                ...
                <td class="text-center">
                    ${item.PSTATUSID === 19 || item.PSTATUSID === 23
                ? `<a href="/VendorMaster/VendorApprovalFormFin?VMID=${item.EncryptedVendorHeaderId}" title="Approval Request">
